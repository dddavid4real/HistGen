...
class ScaledDotProductAttention(nn.Module):
    ...
    def forward(self, queries, keys, values, attention_mask=None, attention_weights=None):
        ...
        if attention_mask is not None:
            # att = att.masked_fill(attention_mask, -np.inf) #-1e9
            # att = att.masked_fill(attention_mask, -1e9) 
            att = att.masked_fill(attention_mask.to(torch.bool), -1e9)
...
class ScaledDotProductAttentionMemory(nn.Module):
    ...
    def forward(self, queries, keys, values, attention_mask=None, attention_weights=None):
        ...
        if attention_mask is not None:
            # att[:, :, :, :nk] = att[:, :, :, :nk].masked_fill(attention_mask, -np.inf) #-1e9
            att[:, :, :, :nk] = att[:, :, :, :nk].masked_fill(attention_mask, -1e9) #-1e9
        att = torch.softmax(att, -1)
        out = torch.matmul(att, v).permute(0, 2, 1, 3).contiguous().view(b_s, nq, self.h * self.d_v)  # (b_s, nq, h*d_v)
        out = self.fc_o(out)  # (b_s, nq, d_model)
        return out
